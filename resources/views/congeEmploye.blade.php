                ...
                <!-- fin sidebar -->
                    <div class="pcoded-content">
                        <div class="pcoded-inner-content">
                            <div class="main-body">
                            <a href="{{ route('cdi') }}"><button class="btn btn-success float-right">Ajouter</button></a>
                                <h3 class="text-center">Liste de tous les conges enregistrer</h3>
                                    ...
                                    <div class="page-body">
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <th scope="row">{{ $conge->employe_id }}</td>
                                                    <!-- cest l'ID de l'employe pas le conge -->
                                                    <td class=""><a href="{{ url('details/'.$conge->id) }}" title="voir le details de l'employé">{{ $conge->demandeur }}</a></th>
                                                    <td class="">{{ $conge->poste_occuper }}</td>
                                                    <td class="">{{ $conge->type_de_conge }}</td>
                                                    <td class="">{{ $conge->date_de_debut }}</td>
                                                    <td class="">{{ $conge->heure_de_debut }}</td>
                                                    <td class="">{{ $conge->date_de_fin }}</td>
                                                    <td class="">{{ $conge->heure_de_fin }}</td>
                                                    <td class="">{{ $conge->nombre_de_jours }}</td>
                                                    <td class="">{{ $conge->motifs }}</td>
                                                    <td class="text-center" style="display: revert;">
                                                        <a href="{{ url('editConge/'.$conge->id) }}">                                
                                                            <em class="ti-pencil" style="color: blue; font-size: 15px; " title="Modifier"></em>
                                                        </a>
                                                        <a href="{{ url('delete-conge/'.$conge->id) }}" 
                                                            onclick="return confirm('Voulez-vous supprimer ce conge de'+'{{ $conge->demandeur }}')">                                

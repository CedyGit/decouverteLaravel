                ...
                <!-- fin sidebar -->
                    <div class="pcoded-content">
                        <div class="pcoded-inner-content">
                            <div class="main-body">
                                <h3 class="text-center">Liste des congés Conge Payer</h3>
                                <div class="page-wrapper">
                                    <div class="page-body">
                                            ...
                                            <tbody>
                                                <!-- fore -->
                                                @foreach($data as $congepayer)
                                                <tr>
                                                    <th scope="row">{{ $congepayer->employe_id }}</td>
                                                    <!-- cest l'ID de l'employe pas le conge -->
                                                    <td class=""><a href="{{ url('details/'.$congepayer->id) }}" title="voir le details de l'employé">{{ $congepayer->demandeur }}</a></th>
                                                    <td class="">{{ $congepayer->poste_occuper }}</td>
                                                    <td class="">{{ $congepayer->type_de_conge }}</td>
                                                    <td class="">{{ $congepayer->date_de_debut }}</td>
                                                    <td class="">{{ $congepayer->heure_de_debut }}</td>
                                                    <td class="">{{ $congepayer->date_de_fin }}</td>
                                                    <td class="">{{ $congepayer->heure_de_fin }}</td>
                                                    <td class="">{{ $congepayer->nombre_de_jours }}</td>
                                                    <td class="">{{ $congepayer->motifs }}</td>
                                                    <td class="text-center" style="display: revert;">
                                                        <a href="">                                
                                                            <em class="ti-pencil" style="color: blue; font-size: 15px; " title="Modifier"></em>
                                                        </a>
                                                        <a href="{{ url('delete-conge/'.$congepayer->id) }}" 
                                                            onclick="return confirm('Voulez-vous supprimer ce conge de'+'{{ $congepayer->demandeur }}')">                                

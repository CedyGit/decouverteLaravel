                            ...
                            <div class="main-body">
                                <div class="page-wrapper">
                                    ...
                                    <div class="page-body">
                                       <!-- tableau -->
                                        <!-- barre de recherche -->
                                            <form action="{{ url('chercher') }}" method="GET" class="navbar-search navbar-search-light form-inline mr-sm-3">
                                                @csrf
                                                <div id="recherche">
                                                    <input type="search" class="form-control" name="search" placeholder="Chercher des employes">
                                                    <button class="btn btn-info" type="submit"><a href="{{ url('cherche') }}"><i class="ti-search"></i></a> </button>
                                                </div>
                                            </form>
                                       ...
                                       @csrf
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="nomT"><a href="{{ url('attestation/'.$employe->id) }}"title="Creer une attestation">{{ $employe->nom }}</a></td>
                                                    <td class="">{{ $employe->prenom }}</td>
                                                    <td class="">{{ $employe->sexe  }}</td>
                                                    <td class="">{{ $employe->poste  }}</td>
                                                    <td class="">{{ $employe->date_de_recrutement  }}</td>
                                                    <td class="">{{ $employe->status  }}</td>
                                                    <td class="">{{ $employe->matricule  }}</td>
                                                    <td class="text-center" style="display: revert;">
                                                    <a href="{{ url('details/'.$employe->id) }}">                                
                                                        <em class="ti-user" style="color: green; font-size: 15px; " title="Details"></em>
                                                    </a>
                                                    <a href="{{ url('editEmploye/'.$employe->id) }}">                                
                                                        <em class="ti-pencil" style="color: blue; font-size: 15px; " title="Modifier"></em>
                                                    </a>
                                                    <a href="{{ url('delete-employe/'.$employe->id) }}" 
                                                        onclick="return confirm('Voulez-vous supprimer'+' {{ $employe->nom }}'+' {{ $employe->prenom }}')">                                
                                        ...
                                        <!-- pagination -->
                                        <div class="row">{{ $resultat->links() }}</div>
                                    </div>
